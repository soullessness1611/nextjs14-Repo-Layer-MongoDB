...
export class Repository<T> implements IRepository<T> {
  ...
  //Asynchronous find documents in the collection
  async find(
    filter: Partial<T>,
    page: number = 1,
    limit: number = 10,
    ...
    try {
      ...
      // Access the database and the collection, the find documents matching the filter
      ...
      if (error instanceof Error) {
        if (error.message.includes("ECONNREFUSED")) {
          console.error("Failed to connect to the MongoDB. Connection refused");
        } else {
